...
description: vue3-uikit（@easemob-community/uikit-im / uikit-core / uikit-sdk-contract / uikit-sdk-websdk5）本地 tgz 联调流程与 vite 预打包缓存坑。换 uikit 版本、升级后新功能（如黑名单）不生效、dev 页面无变化时使用。
---
...
- 本 demo 通过**本地 tgz** 集成 uikit 四包：`@easemob-community/uikit-im` / `@easemob-community/uikit-core` / `@easemob-community/uikit-sdk-contract` / `@easemob-community/uikit-sdk-websdk5`（uikit-im 3.x 起为双 adapter 架构 D122，SDK 层拆为中立契约 + 适配器两个独立包）：`package.json` 的 `dependencies` 与 `pnpm.overrides` 均以 `file:./easemob-uikit-<pkg>-<version>.tgz` 引用（tgz 位于仓库根目录，**不入库**，已被 `.gitignore` 排除）。
...
- 入口必须 `import '@easemob-community/uikit-im/websdk5'` 注册默认适配器（见 `src/main.ts`），否则 Provider/useClient 无 adapter 可用。
...
1. 在 uikit 源仓库构建并打包（版本号按需修改）：
   ...
   pnpm -F @easemob-community/uikit-sdk-contract build && (cd packages/uikit-sdk-contract && pnpm pack)
   pnpm -F @easemob-community/uikit-core build && (cd packages/uikit-core && pnpm pack)
   pnpm -F @easemob-community/uikit-sdk-websdk5 build && (cd packages/uikit-sdk-websdk5 && pnpm pack)
   pnpm -F @easemob-community/uikit-im build && (cd packages/uikit-im && pnpm pack)
   ```
   产出 `easemob-uikit-{sdk-contract,core,sdk-websdk5,im}-<v>.tgz`（在各自 package 目录下，注意按依赖序构建）。
2. 把四个新 tgz **复制到本 demo 根目录**。
3. 更新 `package.json`：`dependencies` 与 `pnpm.overrides` 中四个 `@easemob-community/uikit-*` 包的 `file:` 指向**新文件名**（版本号变了文件名也会变）。
...
- **验证是否已是新版本**：
  ```bash
  # 已安装产物含黑名单（>0 为新版）
  grep -c EmBlockList node_modules/@easemob-community/uikit-im/dist/easemob-uikit-im.js
  # dev 预打包产物也是新版（重启后 >0）
