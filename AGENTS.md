...
- **vue3-uikit 联调（tgz 方式，当前采用）**：`package.json` 的 dependencies 与 `pnpm.overrides` 均通过 `file:./easemob-uikit-core-1.4.0.tgz` / `file:./easemob-uikit-im-2.6.0.tgz` 引用本地 tgz（位于仓库根目录，不入库）。uikit 源码仓库（`../UIKIT/easemob-uikit-vue/packages/uikit-core|uikit-im`）重新打包产出新 tgz 后，**必须把新打的 tgz 复制到本 demo 根目录、同步更新 `package.json` 依赖与 `pnpm.overrides` 的 tgz 文件名、执行 `pnpm install`，并重启 `pnpm dev`**——换 tgz 后运行中的 dev server 会继续服务旧版本的 vite 预打包缓存（`node_modules/.vite` 不自动刷新，典型症状：黑名单等新功能不出现、样式无变化）。`vite.config.ts` 已内置防呆（检测到已安装 uikit 产物比缓存新时自动清缓存），兜底可 `rm -rf node_modules/.vite` 后重启。**注意：uikit 仓库升级版本（如 2.5.0→2.6.0）后 tgz 文件名也会变，务必用新文件名重打/复制**。完整流程与排查见 skill `uikit-tgz-integration`。
