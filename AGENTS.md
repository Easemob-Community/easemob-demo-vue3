...
- **侧边栏宽度联动**：Demo 侧三页（会话/通讯录/设置）共用 `useSidebarWidth`，未拖拽记忆时按 `DEMO_SIDEBAR_CONFIG.defaultWidth`（360px）定宽渲染——不走 UIKit 的 fluid 弹性基准（`--uikit-sidebar-width: clamp(240px, 25%, 480px)`，宽窗口下 25% 可达 480px，与默认宽度设置不一致、观感过宽）；用户拖拽后写回 px 并持久化（localStorage key `layout_sidebar_width`），此后按手动宽度固定。若本地存有旧的手动宽度记忆，清掉该 key 即恢复默认 360px。
