...
└── src/
    ...
    ├── components/         # 公共组件（图标统一使用 UIKit EmIcon，不再自维护 SVG 图标组件）
...
- **vue3-uikit 集成（npm registry 方式，当前采用）**：uikit 已发布至 npm 官方 registry 的 `easemob-community` 组织下，`package.json` 直接依赖仅两个包：`@easemob-community/uikit-im`（本体，3.7.11 起主入口透出 core 的 `useUserInfoStore` / 快捷键开关与 `UiContact` / `UiGroup` / `UiPresence` 等类型，业务侧 import 一律从 uikit-im 走）、`@easemob-community/uikit-sdk-websdk5`（5.x 适配器，uikit-im 的 peer，且入口必须 `import '@easemob-community/uikit-im/websdk5'` 注册默认适配器，见 `src/main.ts`）；`uikit-core` / `uikit-sdk-contract` 不直接依赖，作为传递依赖自动安装。项目根 `.npmrc` 已把 `@easemob-community` scope 固定指向官方 registry（`https://registry.npmjs.org`），不受全局 npmmirror 镜像影响。升级 uikit 版本只需改版本号 + `pnpm install` + 重启 `pnpm dev`——运行中的 dev server 会继续服务旧版本的 vite 预打包缓存（`node_modules/.vite` 不自动刷新，典型症状：新功能不出现、样式无变化）；`vite.config.ts` 已内置防呆（检测到已安装 uikit 产物比缓存新时自动清缓存），兜底可 `rm -rf node_modules/.vite` 后重启。**本地 tgz 联调为备选方式**（uikit 未发布版本的验证用），完整流程见 skill `uikit-tgz-integration`。
- **侧边栏宽度联动（core 2.8.4 / im 3.7.4 起）**：UIKit `--uikit-sidebar-width` 默认值由固定 360px 改为弹性 `clamp(240px, 25%, 480px)`（边界 token `--uikit-sidebar-min-width` / `--uikit-sidebar-max-width`），`EmResizable` 新增 `fluid` 模式。Demo 侧三页（会话/通讯录/设置）未拖拽记忆时 `useSidebarWidth` 返回 `undefined`、EmResizable 走 fluid 不写内联宽度，侧栏随窗口伸缩；用户拖拽后写回 px 并持久化（localStorage key `layout_sidebar_width`），此后按手动宽度固定。若本地存有旧的手动宽度记忆，清掉该 key 即恢复弹性。
- **已读回执临时关闭（Prod 集群问题止血）**：因正式 Prod 集群已读回执（已读/未读状态变更）问题，Demo 默认双层关闭——① Provider 面板「消息已读回执」总开关 `enableReadReceipt`（`DEMO_PROVIDER_CONFIG.enableReadReceipt: false`，UIKit 3.7.7 新增、3.7.8 收紧的全局特性开关：关闭后不发消息级已读回执，且己方消息 SENT/DELIVERED/READ 状态图标即「已读未读」展示层整体隐藏，气泡旁不再残留任何状态指示，见 `src/App.vue` 绑定）；②「聊天」面板群已读回执开关 `groupReadReceiptEnabled` 默认 `false`（群聊发送侧不再请求 `needReadReceipt`）。Prod 恢复后把两处默认值改回 `true` 即可。
- **聊天区防诈骗提示条**：基于 UIKit `EmChatContainer` `#notice` 插槽（im 3.7.9 新增，位于聊天 header 与消息列表之间）挂载 demo 自研 `AntiFraudBanner`（`src/components/chat/AntiFraudBanner.vue`），PC / H5 均已接入；「点我举报」为成功 toast（对齐 React demo `FraudTip` 交互，无后端举报接口）；关闭仅对当前会话生效，切换会话重新展示。
- **聊天区水印**：基于 UIKit `EmChatContainer` `#watermark` 插槽（UIKit 提供铺满聊天区、`pointer-events: none` 的覆盖层壳）挂载 demo 自研 `ChatWatermark`（`src/components/chat/ChatWatermark.vue`，平铺斜排低透明度文本），PC / H5 均已接入、常驻展示；文案走 i18n key `chat.watermark`（默认「环信Demo 仅供参考，谨防诈骗！」）。im 3.7.10 起覆盖层收窄到聊天内容区（header / 通知条 / 消息列表），不再覆盖输入框与多选操作栏，无会话空状态不渲染水印。
- **页面级空状态文案（Figma 新 Demo 空状态 1336:6814 / 1336:6983）**：统一由 demo 自研 `EmptyStateNotice`（`src/components/common/EmptyStateNotice.vue`）渲染两行提示（主文案 24px Medium + 副文案 13px，颜色 token `--color-text-quaternary`，文案走 i18n `emptyState.title` / `emptyState.tip`）。聊天页经 `EmChatContainer` `#empty` 插槽注入（图标沿用 UIKit EmEmpty 自带 `bubble/rect/2`，页面 deep 覆盖为 100px 并经 `--uikit-empty-icon-color` 下发颜色）；通讯录 PC 主区未选中联系人/群组时的占位为 demo 自绘，直接挂载并传 `icon="person/double"`。仅 PC 布局有空态场景（H5 未选中时展示列表）。
...
- **UIKit 特性联动（字号 / 密度 / 间距 / 圆角 / 主色）**：`src/composables/useUIKitConfig.ts` 是唯一联动层，watch UIKit 主题 store 并回写 Demo 侧 CSS 变量 / data 属性——`fontSizeScale` → `--demo-font-scale`、`density` → `html[data-demo-density]`、`containerGap` → `--demo-container-gap`、`componentsShape` → `data-demo-components-shape` + `--demo-component-radius`、`primaryColorHsl` → `--color-primary` 系。**新增 Demo 样式时字号一律写 `calc(Npx * var(--demo-font-scale, 1))`，禁止写死 `font-size: Npx`**（行高同理，无单位行高如 1.5 除外）；主题 store 持久化缓存（localStorage `easemob_uikit_theme`）的旧字段缺失补齐走 `patchLegacyThemeStorage`，不要无条件 set 覆盖用户已调值。
