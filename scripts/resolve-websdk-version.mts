 ...
 *
 * 背景：easemob-websdk 是 @easemob-community/uikit-sdk-websdk5 的传递依赖（pnpm 不会提升到根
...
export function resolveWebsdkVersion(fallback = '5.x'): string {
  try {
    ...
    // 先定位 @easemob-community/uikit-sdk-websdk5 包目录（pnpm 虚拟存储内），
    // 再从该目录解析其依赖 easemob-websdk 的入口文件
    const adapterDir = dirname(require.resolve('@easemob-community/uikit-sdk-websdk5'))
