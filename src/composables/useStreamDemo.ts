/**
 * 流式消息模拟器（mock 数据注入，不走真实 SDK onStreamMessage）。
 *
 * 用途：
 * 1. 演示 UIKit 流式消息链路（store 覆盖更新 + 纯文本流式渲染 + markdown 插件气泡）；
 * 2. Mock AI 助手：发送文本消息后自动注入一条 markdown 流式回复，模拟端到端 AI 对话。
 *
 * 说明：真实环境由 SDK `onStreamMessage` 事件驱动，本模拟器只向 messageStore 注入分片，
 * 完整走 store 响应式更新链路。
 */
import {
  MESSAGE_STATUS,
...
import type { ConversationTypeValue, UiMessage } from '@easemob-community/uikit-im'

/** Mock AI 助手身份 */
export const AI_ASSISTANT_ID = 'ai_assistant'
export const AI_ASSISTANT_NICKNAME = 'AI 助手'

/** 注入 / 更新消息所需的 message store 方法子集 */
  ...
  const baseMsg: UiMessage = {
    ...
    ext: {
      ease_chat_uikit_user_info: {
        nickname: from === AI_ASSISTANT_ID ? AI_ASSISTANT_NICKNAME : '',
      },
    },
    timestamp: Date.now(),
    body: { content: '' },
...
/**
 * 在指定会话注入一条 markdown 流式演示消息。
 */
export function runMarkdownStreamDemo(
  store: StreamMessageStore,
  options: Pick<StreamSimulationOptions, 'conversationId' | 'conversationType' | 'to'>,
): () => void {
...
/**
 * 在指定会话注入一条异常结束的 markdown 流式演示消息。
 */
export function runMarkdownStreamErrorDemo(
  store: StreamMessageStore,
  options: Pick<StreamSimulationOptions, 'conversationId' | 'conversationType' | 'to'>,
): () => void {
  return simulateStreamMessage(store, {
